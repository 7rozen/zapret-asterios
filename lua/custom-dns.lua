...
-- Description : Lua модуль для перехвата DNS-запросов и ответов, с возможностью подмены IP-адресов на основе локального hosts-файла или перенаправления на заданный DNS-сервер.
...
local function load_dns()
    local path = "/usr/bin/configs/dns"
...
local function load_hosts()
    local path = "/usr/bin/configs/hosts"
    local f = io.open(path, "r")
    if f then
        local count = 0
