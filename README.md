...
### Быстрый старт
[Скачиваем архив](https://github.com/7rozen/zapret-asterios/archive/refs/heads/main.zip), распаковываем и запускаем один из пресетов: `preset-all-*.cmd`, `preset-asterios-*.cmd`, `preset-asteriosgame-*.cmd`.

* Пресеты all используют стратегии обхода блокировок для всего трафика.
* Пресеты asterios используют стратегии обхода блокировок только для Астериоса (сайта, апдейтера и игры).
* Пресеты asteriosgame используют стратегии обхода блокировок только для игры.
...
* В корневую папку добавлены 9 пресетов для обхода блокировок.
* В папку `fakes` добавлены пейлоады для фейковых пакетов.
* В папку `lists` добавлены списки ip и доменов для фильтрации трафика.
* Добавлена папка `configs`, в которой хранятся текстовые файлы с параметрами запуска для winws2.
* В папку lua добавлен модуль `custom-dns.lua`, позволяющий настраивать dns и файл hosts не трогая системные настройки.
...
* `configs\`- папка с конфигурационными файлами для winws2
...
* `preset-asteriosgame-*.cmd` - пресеты, которые использует правила обхода только для игры.
