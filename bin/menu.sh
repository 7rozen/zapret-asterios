...
ROOT_DIR="$(dirname "${BIN_DIR_M}")"
cd "${ROOT_DIR}" || exit 1

PRESETS_DIR="./presets"
TMP_DIR="./tmp"

STATE_FILE="${BIN_DIR}/configs/state"
DNS_FILE="${BIN_DIR}/configs/dns"
DNS_LIST_FILE="${BIN_DIR}/configs/dns_list"
REDIRECT_FILE="${BIN_DIR}/configs/redirect"
SERVERS_FILE="${BIN_DIR}/configs/servers"
WINWS2="${BIN_DIR}/winws2.exe"
...
FOOTER_LINES=5
FOOTER_TEXT_LINES=3
BODY_LINES=$(( UI_LINES - HEADER_LINES - FOOTER_LINES - 2 ))
...
declare -A SERVICE_PID_FILE=(
    [zapret]="${TMP_DIR}/zapret.pid"
    [redirect]="${TMP_DIR}/redirect.pid"
    [dns]="${TMP_DIR}/dns.pid"
)
...
load_state() {
    ...
    if [[ -f "${STATE_FILE}" ]]; then
        ...
        local svc
        for svc in zapret redirect dns; do
...
service_is_running() {
    ...
    [[ "$_pid" =~ ^[0-9]+$ ]] || { rm -f "$pidfile"; return 1; }
...
service_start() {
    ...
    [[ -f "$WINWS2" && -f "$preset" ]] || return 1

    local attempt
    for (( attempt=1; attempt<=3; attempt++ )); do
        rm -f "${SERVICE_PID_FILE[$svc]}"
        [[ "$svc" == "dns" ]] && cygstart --hide ipconfig /flushdns
        cygstart --hide "$WINWS2" --daemon --pidfile="${SERVICE_PID_FILE[$svc]}" @"$preset"
...
draw_screen_preset() {
    ...
    local max_visible=$(( BODY_LINES - 3 ))
